...
###base_path = "/trinity/home/skatz/PROJECTS/ChemSpaceAL-slurm"  ##os.getcwd()
os.chdir('..')
base_path = f"{os.getcwd()}"
dd_path = os.path.join(base_path, "content/DiffDock")
...
def get_top_poses(ligands_csv: str, protein_pdb_path: str, save_pose_path: str):
    ...
    for i in pbar:
        ...
        with open(os.path.join(base_path, "content/input_protein_ligand.csv"), "w") as out:
        ...
        subprocess.run(['python', 'datasets/esm_embedding_preparation.py', "--protein_ligand_csv", "../input_protein_ligand.csv", "--out_file", "data/prepared_for_esm.fasta"])
        ...
        # ESM Extraction
        #os.system("esm/scripts/extract.py esm2_t33_650M_UR50D /content/DiffDock/data/prepared_for_esm.fasta /content/DiffDock/data/esm2_output --repr_layers 33 --include per_tok --truncation_seq_length 30000")

        # Inference
        #os.system("inference.py --protein_ligand_csv /content/input_protein_ligand.csv --out_dir /content/DiffDock/results/user_predictions_small --inference_steps 20 --samples_per_complex 10 --batch_size 6")
